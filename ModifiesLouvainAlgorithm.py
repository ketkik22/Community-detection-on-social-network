...
#     
    for idx, eb in enumerate(ebList):
        ...
        if(g.es[idx].tuple[0]!= g.es[idx].tuple[1] and g.are_connected(g.es[idx].tuple[0], g.es[idx].tuple[1])):
            tupleId = idx
...
#      
    ...
    componentList = g1.clusters(mode=STRONG)
    #print "*"
    #print "*"
    print "*"
    #print componentList
...
#   
    g = g2.copy()
    #g.delete_edges(alreadyUsedTuples1)
    #plot(g, "G%d.png" %idx1, **visual_style)
    modularity = g1.modularity(componentList.membership)
    print ">>>>>>>>>>>>>>>>>>>>>>>>>>>>",modularity
    g.contract_vertices(componentList.membership, combine_attrs=max)
    #componentList.membership
